...
app.post('/api/process-image', upload.single('image'), async (req, res) => {
  ...
  let inputPath, tempPath, outputPath;
  try {
    ...
    const response = {
      file: {
        data: base64File,
        filename: `processed-image.${format}`,
    ...
    const paths = [inputPath, tempPath, outputPath].filter(Boolean);
    console.log('Cleaning up files:', paths);
    await delay(1000); // Increased to 1 second
    await Promise.all(
      paths.map(async (p) => {
        const deleted = await unlinkWithRetry(p, 5, 1000); // 5 retries, 1s delay
        if (!deleted) {
          console.warn(`File ${p} could not be deleted and will remain in the directory.`);
        }
      })
    );
